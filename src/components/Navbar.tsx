...
// Avatar seguro
const EmailAvatar = ({ email }: { email?: string }) => {
  const initial = (email || "U").charAt(0).toUpperCase();
  return (
    <div className="w-12 h-12 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white text-xl font-bold shadow-md">
      {initial}
    </div>
  );
};
...
const RoleBadge = ({ role }: { role: string }) => {
  const config = {
    admin: { label: "ADMIN", icon: <Shield size={12} />, bg: "bg-red-100", text: "text-red-800" },
    instructor: { label: "INSTRUCTOR", icon: <Presentation size={12} />, bg: "bg-blue-100", text: "text-blue-800" },
    user: { label: "ESTUDIANTE", icon: <GraduationCap size={12} />, bg: "bg-green-100", text: "text-green-800" },
  };
  const { label, icon, bg, text } = config[role as keyof typeof config] || config.user;

  return (
    <span className={`ml-2 inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded-full ${bg} ${text}`}>
      {icon} {label}
    </span>
  );
};
...
export default function Navbar() {
  const { user, status, isAdmin, isInstructor, isUser, logout } = useAuth();
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="w-32 h-8 bg-gray-200 rounded-full animate-pulse hidden md:block" />
            </div>
          </div>
        </div>
      </nav>
          ...
          <div className="flex items-center space-x-3">
            ...
            {status === "authenticated" && user ? (
              <div className="hidden md:block relative" ref={userRef}>
                ...
                >
                  {user.image ? (
                    <Image src={user.image} alt={user.name || "User"} width={36} height={36} className="rounded-full border-2 border-purple-200" />
                  ) : (
                  ...
                  <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden">
                    <div className="p-4 bg-gradient-to-r from-purple-50 to-pink-50 border-b">
                      <div className="flex items-center gap-3">
                        {user.image ? (
                          <Image src={user.image} alt="User" width={48} height={48} className="rounded-full" />
                        ) : (
                          <EmailAvatar email={user.email} />
                        )}
                    ...
                    <div className="py-2">
                      <Link href="/dashboard" className="flex items-center gap-3 px-4 py-3 hover:bg-purple-50 text-gray-700 transition-colors" onClick={() => setIsUserOpen(false)}>
                        <User className="w-5 h-5" /> Mis cursos
                      </Link>
                      {isAdmin && (
                        <>
                          <Link
                            href="/dashboard/usuarios"
                            className="flex items-center gap-3 px-4 py-3 hover:bg-purple-50 text-gray-700 transition-colors rounded-lg"
                          ...
                          <Link
                            ...
                            className="flex items-center gap-3 px-4 py-3 hover:bg-purple-50 text-blue-600 hover:text-blue-800 transition-colors rounded-lg"
                            onClick={() => setIsUserOpen(false)}
                          >
                      ...
                      {isUser && (
                        <>
                          <Link href="/dashboard/student/available" className="text-gray-700 hover:text-blue-600 flex items-center gap-1">
                            <Home className="w-4 h-4" />
                            Disponibles
                          </Link>
                          <Link href="/dashboard/student/my" className="text-gray-700 hover:text-blue-600 flex items-center gap-1">
                      ...
                      {isInstructor && (
                        <Link href="/dashboard/instructor/create" className="text-gray-700 hover:text-blue-600 flex items-center gap-1">
                      ...
                      {isAdmin && (
                        <Link href="/admin/payments" className="text-gray-700 hover:text-red-600 flex items-center gap-1">
                          <Shield className="w-4 h-4" />
                          Admin
                        </Link>
                      )}
                      <Link href="/profile" className="flex items-center gap-3 px-4 py-3 hover:bg-purple-50 text-gray-700 transition-colors" onClick={() => setIsUserOpen(false)}>
                    ...
                    <div className="border-t pt-2">
                      <button onClick={() => signOut({ callbackUrl: "/" })} className="w-full flex items-center gap-3 px-4 py-3 text-red-600 hover:bg-red-50 transition-colors">
            ...
            <div className="hidden md:block relative" ref={langRef}>
              <button onClick={() => setIsLangOpen(!isLangOpen)} className="p-2 text-gray-700 hover:text-purple-600 rounded-lg transition-colors">
          ...
          <div className="px-4 pt-4 pb-3 space-y-4">
            ...
            <div className="space-y-1">
              ...
              {categories.slice(0, 5).map((cat) => (
                <Link key={cat.name} href={cat.href} className="flex items-center gap-2 py-2 text-sm text-gray-700 hover:text-purple-600" onClick={() => setIsMenuOpen(false)}>
            ...
            {status === "unauthenticated" && (
              <div className="flex flex-col space-y-2 pt-2">
                <Link href="/auth/login" className="px-4 py-2 text-center text-sm font-medium text-gray-700 bg-gray-100 rounded-md">
                  Iniciar sesión
                </Link>
                <Link href="/auth/register" className="px-4 py-2 text-center bg-purple-600 text-white text-sm font-medium rounded-md hover:bg-purple-700">
                  Registrarse
                </Link>
              </div>
            )}
